...
3. Use \`get_page\` to read current content + version
...
5. Every write requires the correct \`baseVersion\` (from the \`version\` or \`pageVersions[ref]\` field returned by the matching read tool)
...
- **Read**: \`list_projects\`, \`get_top\`, \`get_graph\`, \`get_page\`, \`search_nodes\`, \`list_source_files\`, \`read_source_files\`, \`list_history\`
- **Write**: \`patch_page\` (preferred), \`update_page\`, \`update_node\`, \`update_graph_meta\`, \`create_node\`, \`delete_node\`, \`update_top\` (description only), \`revert\`

All read tools return a \`version\` field — pass it directly as \`baseVersion\` in the matching write tool.
...
- Do NOT repeat mechanical tool-call details (\`baseVersion\`, \`fromVersion\`) — those are implementation noise
