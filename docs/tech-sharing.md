# ACCEED 技术分享：面向代码仓库的多 Agent 文档生成流水线

## 一、分享摘要

ACCEED 的目标不是再做一个“把仓库丢给大模型，然后生成一批 Markdown”的工具，而是把代码文档生成拆成一条可恢复、可校验、可增量维护的工程流水线。
...
ACCEED 的切入点是：**把文档生成当作编译流水线，而不是一次模型调用。**
...
ACCEED 的主流程由 Arranger 状态机驱动，整体管线如下：
...
ACCEED 没有引入 Redis、BullMQ 或专用 Agent 编排框架。它把文档树中的每个模块节点直接视为一个任务，任务状态就写在对应的 JSON 文件中。
...
ACCEED 的输出不是平铺 Markdown，而是一棵可导航、可验证、可被 Agent 按需读取的文档树。
...
ACCEED 把这些问题交给 Checker 独立校验。Checker 只检查 Scaffold 和 Decomposer 的图结构，不检查 Writer 生成的 Markdown 正文。
...
文档生成完成后，真正的价值在于能否被持续使用。ACCEED 通过 MCP Server 暴露文档读写工具，再通过 `doc-drill` Skill 给 Code Agent 一套稳定的导航协议。
...
ACCEED 会在目标仓库安装 Codex 项目级 `.codex/skills/doc-drill/SKILL.md`，并写入 Codex / Claude Code 对应的 MCP 配置。
...
初次生成只解决“从无到有”，文档真正的成本在后续维护。ACCEED 的增量更新由 PrUpdater Agent 和 `UpdateQueuePanel` 共同完成。
...
ACCEED 的前端基于 Vue 3 和 TypeScript，核心页面包括：
...
在前端输入目标仓库 Git URL 后，ACCEED 会执行：
...
ACCEED 的实现刻意保留了几个明确的扩展边界，方便接入不同模型、修改 prompt 或扩展文档工具。
...
ACCEED 解决的问题不是“用 AI 写一篇文档”，而是“让代码文档具备工程系统应有的生命周期能力”。
