...
**Google CodeWiki** [17] 产出模块树 JSON、每模块 Markdown 页面和架构图。其模块树的实际结构为 `{模块名: {path, components, children}}`——本质上是一个**按目录路径分组的组件清单**，不编码任何模块间关系（无边、无依赖类型、无交互语义）。其局限：(1) 模块间的调用、数据流、事件等架构关系完全缺失，Agent 无法从模块树推断跨模块交互；(2) `components` 字段为文件路径::类名的平铺列表，缺乏 AD-IR 中子集-划分约束级别的源码作用域绑定；(3) 产出物为静态文件集合，不支持增量更新和 Agent 写回。
