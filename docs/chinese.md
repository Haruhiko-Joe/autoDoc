...
将整个代码仓库转化为层次化、可导航的文档体系，是近年来文档生成领域的核心方向。HGEN 将这一问题表述为"从源码动态生成文档层级"，通过六阶段流水线在工业项目上验证了层次化文档的可行性 [16]。Google CodeWiki 提出仓库级文档框架，强调层次化 agentic 分解与多语言支持 [17]。SWD-Bench 则从评估角度出发，通过 QA 和 feature-driven development 任务衡量仓库文档的实际效用，发现高质量文档能显著提升 Agent 的 issue-solving 能力 [11]。

然而，现有仓库级文档生成工作普遍将生成后的文档视为**静态输出终点**：文档一旦生成便不再被系统内其他组件程序化消费和写回。ACCEED 与这条线的核心区别不在于"也有层次结构"，而在于将层次结构直接暴露为可交互、可被 Agent 编排调用的对象模型——Agent 可通过标准化查询-修改接口逐层导航并对文档执行结构化写回。
...
上述工作将仓库理解作为**一次性推理过程**——理解结果内化于模型的上下文或嵌入中，无法被后续会话复用。ACCEED 将仓库理解的成果**外化为持久化文档工件**，使其能够被多个 Agent 会话反复消费、修改和维护，将理解成本摊薄到长期使用中。
...
然而，现有工作在"持续维护"和"Agent 接口"上呈分离状态：维护侧工作关注如何高效刷新文档内容，接口侧工作关注如何让 Agent 调用外部工具——但没有工作将两者统一为同一系统的两个面。ACCEED 通过将生成与更新共享同一查询-修改接口，使文档系统既是生成管线的输出终点，也是 Agent 的读写工作空间，从而将 PR 驱动的变更感知维护与 Agent 原生的程序化访问融合为同一架构。

### 小结

综合上述分析，现有工作已分别在层次化文档生成 [16,17,11]、代理式仓库理解 [3,4,9,10] 和 Agent 工具接口 [18] 三个方向取得进展，但尚无工作将三者统一：将仓库级层次化文档形式化为 Agent 可读可写的中间表示，通过产物即状态原则驱动生成，并以 PR 粒度的增量维护保持文档与代码的持续同步。ACCEED 正是填补这一交叉空白。
