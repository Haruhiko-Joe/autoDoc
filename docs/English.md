...
**Google CodeWiki** [17] outputs a module-tree JSON, per-module Markdown pages, and architecture diagrams. While advancing beyond HGEN, it has critical shortcomings: (1) inter-module relationships encode only a single type `depends_on`, unable to distinguish calls, data-flow, events, inheritance, and composition — a distinction essential for agents' cross-module reasoning; (2) no source-code scope bindings exist between documentation nodes and source files, precluding bidirectional traceability and change-impact localization; (3) the output is a static file collection, supporting neither incremental updates nor agent write-back.
