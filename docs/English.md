...
Transforming an entire code repository into a hierarchical, navigable documentation system has become a central direction in documentation generation. HGEN formulates this as "dynamically generating documentation hierarchies from source code," validating hierarchical documentation through a six-stage pipeline on industrial projects [16]. Google CodeWiki proposes a repository-level documentation framework emphasizing hierarchical agentic decomposition and multi-language support [17]. SWD-Bench approaches this from an evaluation perspective, measuring repository documentation utility through QA and feature-driven development tasks, finding that high-quality documentation significantly improves agents' issue-solving capabilities [11].

However, existing repository-level documentation generation work uniformly treats generated documentation as a **static output endpoint**: once produced, documentation is not programmatically consumed or written back by other system components. ACCEED's core distinction from this line is not "also having hierarchy," but exposing that hierarchy as an interactive, agent-orchestrable object model—agents can navigate layer by layer and perform structured write-back through standardized query-mutate interfaces.
...
These works treat repository understanding as a **one-time reasoning process**—understanding results are internalized within the model's context or embeddings and cannot be reused by subsequent sessions. ACCEED **externalizes repository understanding into persistent document artifacts**, enabling multiple agent sessions to repeatedly consume, modify, and maintain them, amortizing the understanding cost over long-term use.
...
However, existing work treats "continuous maintenance" and "agent interfaces" as separate concerns: maintenance-side work focuses on efficiently refreshing documentation content, while interface-side work focuses on enabling agents to invoke external tools—but no work unifies both as two facets of the same system. ACCEED, by sharing the same query-mutate interface between generation and updates, makes the documentation system simultaneously the output endpoint of the generation pipeline and the read-write workspace for agents, fusing PR-driven change-aware maintenance with agent-native programmatic access into a single architecture.

### Summary

Synthesizing the above analysis, existing work has made progress along three directions: hierarchical documentation generation [16,17,11], agentic repository understanding [3,4,9,10], and agent tool interfaces [18]. However, no work unifies all three: formalizing repository-level hierarchical documentation as an agent-readable, agent-writable intermediate representation, driving generation through the artifact-as-state principle, and maintaining documentation-code synchronization through PR-granularity incremental updates. ACCEED fills precisely this intersection.
