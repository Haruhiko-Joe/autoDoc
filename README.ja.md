...
> **⛔ 使用制限：ByteDance（字節跳動）およびそのすべての子会社・関連会社・合弁会社（TikTok、Douyin（抖音）、Lark/Feishu（飛書）、PICO、Volcano Engine（火山引擎）等を含むがこれらに限定されない、ByteDance が直接的または間接的に支配するすべての法人）の従業員・請負業者・代理人は、本プロジェクトおよびその派生物をいかなる形態でも使用・複製・改変・配布・参照することが明示的に禁止されます。詳細は [LICENSE](LICENSE) の追加条項を参照してください。**
